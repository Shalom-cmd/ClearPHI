...
def health():
    return jsonify({"status": "ok", "service": "clearPHI"}), 200
...
def download_file():
    ...
    # Security — must be inside output_docs
...
# ── Legacy endpoints (AnythingLLM / text-only) ─────────────────
@app.route("/deidentify", methods=["POST"])
def deidentify():
    # Legacy endpoint — requires pdfplumber and presidio (optional dependencies)
    try:
        from src.engine.extractor import extract_text
        from src.engine.deid import deidentify_text, save_redaction_log
    except ImportError as e:
        return jsonify({"error": f"Legacy endpoint unavailable: {e}"}), 501
